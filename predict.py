...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    for i, (input, target, batch_cif_ids) in enumerate(val_loader):
        ...
        # compute output
        output, latent, latent_final = model(*input_var)
        ...
        if model_args.task == 'regression':
            ...
            if test:
                ...
                latents += latent.data.cpu().numpy().tolist()
                latents_final += latent_final.data.cpu().numpy().tolist()
    ...
    if test:
        ...
        with open('test_results.csv', 'w') as f:
            writer = csv.writer(f)
            for cif_id, target, pred, embedding, embedding_final in zip(
                    test_cif_ids, test_targets, test_preds, latents,
                    latents_final):
                writer.writerow(
                    (cif_id, target, pred, embedding, embedding_final))
    else:
        star_label = '*'
    if model_args.task == 'regression':
