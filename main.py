...
parser.add_argument('--lr-milestones',
                    ...
                    type=int,
                    metavar='N',
                    help='milestones for scheduler (default: '
                    '[100])')
...
parser.add_argument('--h-fea-len',
                    default=128,
                    type=int,
                    metavar='N',
                    help='number of hidden features after pooling')
parser.add_argument('--n-conv',
                    default=3,
                    type=int,
                    metavar='N',
                    help='number of conv layers')
parser.add_argument('--n-h',
                    default=1,
                    type=int,
                    metavar='N',
...
def validate(val_loader, model, criterion, normalizer, test=False):
    ...
    if test:
        ...
        with open('test_results.csv', 'w') as f:
            writer = csv.writer(f)
            for cif_id, target, pred, latent in zip(test_cif_ids, test_targets,
                                                    test_preds, latent):
                writer.writerow(
                    (cif_id, target, pred, list(latent.data.cpu().numpy())))
    else:
        star_label = '*'
    if args.task == 'regression':
